...
JSON_PATH = os.path.join("results", "step8_summary_statistics.json")
OUT_DIR = os.path.join("..", "figures-txt")
...
def fig_refusal_rates():
    ...
    values = [
        ...
        rr["usu_baseline_refusal_rate"],
        rr["usu_ablated_refusal_rate"],
        rr["sss_ablated_refusal_rate"],
    ]
    counts = [
        f"{rr['suu_num_refused']}/{sc['suu_total_inferred']}",
        f"{rr['usu_num_refused']}/{sc['usu_total_inferred']}",
        f"{int(rr['usu_ablated_refusal_rate'] * sc['usu_ablated_inferred'])}/{sc['usu_ablated_inferred']}",
    ...
    x1, x2 = 1, 2
    y1, y2 = rr["usu_baseline_refusal_rate"], rr["usu_ablated_refusal_rate"]
    bracket_x = 3.45
    ...
    ax.text(
        bracket_x + 0.08,
        mid_y,
        f"$\\Delta$ = {rr['usu_ablation_delta']:.0%}",
        ha="left",
        va="center",
        fontsize=8,
    )
...
def fig_layer_dot_products():
    ...
    ax.set_title("Layer-wise Refusal Vector Alignment with Image Activations")
...
def fig_pipeline():
    ...
    steps = [
        (0.5, 7.2, "Step 1: SUU Text-Only Inference (150 samples)", 4.0, 0.7),
        ...
        (0.5, 3.6, "Step 3: USU Image-Only Baseline (950 samples)", 4.0, 0.7),
        (5.3, 4.8, "Step 4: Similarity Analysis", 4.2, 0.7),
        (5.3, 3.6, "Step 5: Ablated USU Inference", 4.2, 0.7),
        (2.9, 2.1, "Steps 6--8: Addition, KL Divergence & Summary", 4.2, 0.7),
    ]
