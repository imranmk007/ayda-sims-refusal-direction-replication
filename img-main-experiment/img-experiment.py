...
# Refusal vector derived from harmful IMAGES (image+neutral_text) vs safe IMAGES (image+neutral_text).
# Applied cross-modally to harmful TEXT and safe TEXT inputs.

MODEL_ID = "Qwen/Qwen3.5-9B"
DEVICE = "cuda"
DTYPE = torch.float16
N_TEXT_SAMPLES = 1000
N_HARMFUL_IMAGE_SAMPLES = 500
N_SAFE_IMAGE_SAMPLES = 100
LAYERS_START = 13
LAYERS_END = 31
OUTPUT_DIR = "results"
...
def load_harmful_text(n=N_TEXT_SAMPLES):
    ...
    text_col = next(c for c in hb.column_names if c.lower() in ("prompt", "behavior", "text", "query"))
...
def compute_kl_divergence(sss_logit_dir, ablated_logit_dir, n_samples, filename):
    ...
    plt.title("KL Divergence: Safe Text Original vs. Ablated Logits")
    plt.legend()
    plt.tight_layout()
...
def step1_harmful_image_inference(model, processor, gemini_model):
    """FigStep harmful images (image+neutral_text) — collect activations, filter refused."""
    print(
        "\n========== STEP 1: Harmful Image Inference (Refusal Vector Source) =========="
    )
    ...
    if failed_count > 0:
        print(f"  WARNING: {failed_count} judge() calls failed (None) — treated as non-refusal")
    refusal_rate = len(refused_indices) / len(judgments)
    print(
        f"  Refused: {len(refused_indices)}/{len(judgments)} ({refusal_rate:.1%})"
    )
    ...
    results = {
        ...
        "total_inferred": N_HARMFUL_IMAGE_SAMPLES,
    }
    torch.save(results, os.path.join(OUTPUT_DIR, "step1_harmful_image_results.pt"))
    print("  Saved to step1_harmful_image_results.pt")
    return results


def step2_safe_image_and_refusal_vector(model, processor):
    """Safe images (image+neutral_text) — compute safe mean, derive refusal vector."""
    print("\n========== STEP 2: Safe Image Inference + Refusal Vector ==========")
    samples = load_safe_images()
    layer_sums = {}
    layer_count = 0

    for i, sample in enumerate(samples):
        print(f"  Inference {i+1}/{len(samples)}: safe image {i}...")
    ...
    print(
        "  Computing refusal vector (mean refused harmful images - mean safe images)..."
    )
    step1_meta = torch.load(os.path.join(OUTPUT_DIR, "step1_harmful_image_results.pt"))
    refused_indices = step1_meta["refused_indices"]
    print(f"  Using {len(refused_indices)} refused harmful-image activations.")
...
def step3_harmful_text_baseline(model, processor, gemini_model):
    """Harmful text (text-only) — cross-modal baseline refusal rate."""
    print(
        "\n========== STEP 3: Harmful Text Baseline (HarmBench + AdvBench) =========="
    )
    ...
    print(
        f"  Baseline harmful text refusal rate: {refusal_rate:.2%} ({len(refused_indices)}/{len(judgments)})"
    )
    ...
    results = {
        ...
        "refusal_rate": refusal_rate,
        "num_refused": len(refused_indices),
        "total_inferred": N_TEXT_SAMPLES,
    }
    torch.save(
        results, os.path.join(OUTPUT_DIR, "step3_harmful_text_baseline_results.pt")
    )
    print("  Saved to step3_harmful_text_baseline_results.pt")
    return results
...
def step4_similarity_plot():
    ...
    all_indices = list(range(N_TEXT_SAMPLES))
    ...
    mean_sims, all_dots = compute_and_plot_similarity(
        ...
        title="Layer-wise Similarity: Image-Derived Refusal Vector vs. Text Activations",
    )
    gc.collect()
...
def step5_ablated_harmful_text(model, processor, gemini_model):
    """Ablated harmful text inference — jailbreak test (should reduce refusal rate)."""
    print("\n========== STEP 5: Ablated Inference (Harmful Text) ==========")
    refusal_vectors = torch.load(os.path.join(OUTPUT_DIR, "step2_refusal_vectors.pt"))
    refusal_vectors = {k: v.to(DEVICE).to(DTYPE) for k, v in refusal_vectors.items()}
    step3 = torch.load(
        os.path.join(OUTPUT_DIR, "step3_harmful_text_baseline_results.pt")
    )
    ...
    if step3_queries is not None:
        ...
        if current_queries != step3_queries:
            raise RuntimeError(
                "Step 5: Sample order doesn't match step 3. Re-run from step 3."
            )
    ...
    for i, sample in enumerate(samples):
        print(f"  Ablated {i+1}/{len(samples)}: {sample['query'][:60]}...")
    ...
    torch.save(
        results, os.path.join(OUTPUT_DIR, "step5_ablated_harmful_text_results.pt")
    )
    print("  Saved to step5_ablated_harmful_text_results.pt")
    return results


def step6_addition_inference(model, processor, gemini_model):
    """Addition inference: harmful text (amplify) + safe text (induce refusal)."""
    ...
    if step3_queries is not None:
        ...
        if current_queries != step3_queries:
            raise RuntimeError(
                "Step 6: Sample order doesn't match step 3. Re-run from step 3."
            )
        print("  Verified: sample ordering matches step 3.")
    ...
    for i, sample in enumerate(samples):
        print(f"  Harmful added {i+1}/{len(samples)}: {sample['query'][:60]}...")
    ...
    print("\n  Part 2: Safe text + addition...")
    samples = load_safe_text()
    safe_added_responses = []
    safe_added_judgments = []

    for i, sample in enumerate(samples):
        print(f"  Safe added {i+1}/{len(samples)}: {sample['query'][:60]}...")
        response = run_inference_added(
            model,
            processor,
            query=sample["query"],
...
def step7_safe_text_kl_data(model, processor, gemini_model):
    """Safe text baseline + ablated — collect logits for KL divergence."""
    print("\n========== STEP 7: Safe Text KL Data (Alpaca) ==========")
    refusal_vectors = torch.load(os.path.join(OUTPUT_DIR, "step2_refusal_vectors.pt"))
    refusal_vectors = {k: v.to(DEVICE).to(DTYPE) for k, v in refusal_vectors.items()}

    samples = load_safe_text()

    print("\n  Part 1: Normal safe text inference (collect logits)...")
    safe_responses = []
    safe_judgments = []

    for i, sample in enumerate(samples):
        print(f"  Normal {i+1}/{len(samples)}: {sample['query'][:60]}...")
    ...
    print(f"  Safe text baseline refusal rate: {safe_refusal_rate:.2%} (should be ~0%)")

    print("\n  Part 2: Ablated safe text inference (collect logits for KL)...")
    ablated_responses = []
    ablated_judgments = []
    ...
    print(f"  Ablated safe text refusal rate: {ablated_refusal_rate:.2%}")
    ...
    torch.save(results, os.path.join(OUTPUT_DIR, "step7_safe_text_results.pt"))
    print("  Saved to step7_safe_text_results.pt")
    return results
...
def step8_kl_and_summary():
    ...
    mean_kl, kl_values = compute_kl_divergence(
        ...
        n_samples=N_TEXT_SAMPLES,
        filename="step8_kl_divergence.png",
    )

    step1 = torch.load(os.path.join(OUTPUT_DIR, "step1_harmful_image_results.pt"))
    ...
    step5 = torch.load(
        os.path.join(OUTPUT_DIR, "step5_ablated_harmful_text_results.pt")
    )
    step6 = torch.load(os.path.join(OUTPUT_DIR, "step6_addition_results.pt"))
    step7 = torch.load(os.path.join(OUTPUT_DIR, "step7_safe_text_results.pt"))

    harmful_image_refusal_rate = step1["refusal_rate"]
    harmful_text_baseline_rate = step3["refusal_rate"]
    harmful_text_ablated_rate = step5["ablated_refusal_rate"]
    harmful_text_added_rate = step6["harmful_added"]["refusal_rate"]
    safe_text_baseline_rate = step7["safe_refusal_rate"]
    safe_text_added_rate = step6["safe_added"]["refusal_rate"]
    safe_text_ablated_rate = step7["ablated_refusal_rate"]
    ...
    rates = [
        harmful_image_refusal_rate,
        harmful_text_baseline_rate,
        harmful_text_ablated_rate,
        harmful_text_added_rate,
        safe_text_baseline_rate,
        safe_text_added_rate,
        safe_text_ablated_rate,
    ]
    colors = [
        "#8172b2",
        "#4c72b0",
    ...
    ax.set_ylim(0, 105)
    ax.set_title(
        "Refusal Rates — Image-Derived Refusal Vector Applied Cross-Modally to Text Inputs\n"
        "(green = vector subtracted, red = vector added, purple = vector source)",
        fontsize=11,
    )
    ...
    stats = {
        "experiment": "image-derived refusal vector applied cross-modally to text inputs",
        "sample_counts": {
            "n_text_samples": N_TEXT_SAMPLES,
            "n_harmful_image_samples": N_HARMFUL_IMAGE_SAMPLES,
            "n_safe_image_samples": N_SAFE_IMAGE_SAMPLES,
            "harmful_image_refused_kept": step1["num_refused"],
            "harmful_text_refused_baseline": step3["num_refused"],
        },
        "refusal_rates": {
            "harmful_image_baseline": harmful_image_refusal_rate,
            "harmful_text_baseline": harmful_text_baseline_rate,
            "harmful_text_ablated": harmful_text_ablated_rate,
            "harmful_text_ablated_delta": harmful_text_ablated_rate
            - harmful_text_baseline_rate,
            "harmful_text_added": harmful_text_added_rate,
            "harmful_text_added_delta": harmful_text_added_rate
            - harmful_text_baseline_rate,
            "safe_text_baseline": safe_text_baseline_rate,
            "safe_text_added": safe_text_added_rate,
            "safe_text_ablated": safe_text_ablated_rate,
        },
        "addition_experiment": {
            "multiplier": step6["multiplier"],
            "harmful_text_added_rate": harmful_text_added_rate,
            "harmful_text_baseline_rate": harmful_text_baseline_rate,
            "harmful_text_added_delta": harmful_text_added_rate
            - harmful_text_baseline_rate,
            "safe_text_added_rate": safe_text_added_rate,
            "safe_text_baseline_rate": safe_text_baseline_rate,
        },
        "kl_divergence": {
            "mean": mean_kl,
            "std": kl_std,
    ...
    print("\n  ========== REFUSAL RATES ==========")
    print(
        f"  Harmful image (vec src) baseline:  {harmful_image_refusal_rate:.2%}  ({step1['num_refused']}/{N_HARMFUL_IMAGE_SAMPLES})"
    )
    print(
        f"  Harmful text baseline:             {harmful_text_baseline_rate:.2%}  ({step3['num_refused']}/{N_TEXT_SAMPLES})"
    )
    print(
        f"  Harmful text ablated (jailbreak):  {harmful_text_ablated_rate:.2%}  (delta: {harmful_text_ablated_rate - harmful_text_baseline_rate:+.2%})"
    )
    print(
        f"  Harmful text added (amplified):    {harmful_text_added_rate:.2%}  (delta: {harmful_text_added_rate - harmful_text_baseline_rate:+.2%})"
    )
    print(f"  Safe text baseline:                {safe_text_baseline_rate:.2%}")
    print(f"  Safe text added (induced):         {safe_text_added_rate:.2%}")
    print(f"  Safe text ablated (KL ref):        {safe_text_ablated_rate:.2%}")
    print(
...
if __name__ == "__main__":
    ...
    if 1 not in completed:
        step1_harmful_image_inference(model, processor, gemini_model)
        completed.append(1)
    ...
    if 2 not in completed:
        step2_safe_image_and_refusal_vector(model, processor)
        completed.append(2)
        save_checkpoint(completed)
        gc.collect()
        torch.cuda.empty_cache()
    ...
    if 3 not in completed:
        step3_harmful_text_baseline(model, processor, gemini_model)
        completed.append(3)
        save_checkpoint(completed)
        gc.collect()
        torch.cuda.empty_cache()
    ...
    if 5 not in completed:
        step5_ablated_harmful_text(model, processor, gemini_model)
        completed.append(5)
        save_checkpoint(completed)
        gc.collect()
        torch.cuda.empty_cache()
    ...
    if 7 not in completed:
        step7_safe_text_kl_data(model, processor, gemini_model)
        completed.append(7)
        save_checkpoint(completed)
        gc.collect()
        torch.cuda.empty_cache()
